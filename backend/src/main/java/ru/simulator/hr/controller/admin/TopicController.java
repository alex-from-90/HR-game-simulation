...
public class TopicController {
    ...
    public ResponseEntity<List<Topic>> getAllTopics(@RequestParam(required = false) Long chapterId) {
        if (chapterId != null) {
            ...
            if (!topics.isEmpty()) {
                ...
                return new ResponseEntity<>(HttpStatus.OK); // Заглушка. Вернуть 404, если топики по главе не найдены
            }
        } else {
            ...
            if (!topics.isEmpty()) {
                ...
                return new ResponseEntity<>(new ArrayList<>(), HttpStatus.OK); // Вернуть пустой список топиков с кодом 200
            }
        }
    }
    ...
    public ResponseEntity<Topic> createTopic(@RequestBody Topic topic, @RequestParam(required = false) Long chapterId) {
        if (chapterId != null) {
            // Используйте chapterId в вашей логике создания топика, если он передан
            Topic createdTopic = topicService.createTopic(topic, chapterId);
            return new ResponseEntity<>(createdTopic, HttpStatus.CREATED);
        } else {
            // Обработка сценария, если chapterId не был передан
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST); // Или возвращайте другой код состояния HTTP, в зависимости от вашей логики
        }
    }
